...
def main():
    ...
    # Compile the model if no checkpoint is used or load the model from checkpoint
    if args.check_point is None:
        model.compile(n_classes, arch=args.arch, hidden_units=args.hidden_units,
                drop_p=args.drop_p, learning_rate=args.learning_rate, weights=weights)
    else:
        model.load(args.check_point)

    # Train and save the model if not on evaluate only mode
    if not args.evaluate_only:
        model.train(args.epochs, dataloaders, image_datasets, early_stopping=args.early_stopping)
